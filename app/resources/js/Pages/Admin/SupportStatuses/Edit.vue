...
const form = useForm({
    ...
    name_ar: props.status.name_ar,
    name_fr: props.status.name_fr,
    color: props.status.color || '#6c757d',
...
const submit = () => {
    form.put(route('managit.support-statuses.update', props.status.id));
};
</script>

<template>
    <Head :title="`Edit Status - ${status.name_en}`" />
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        ...
                        <h4 class="page-title">Edit Support Status - {{ status.name_en }}</h4>
                    </div>
                </div>
            </div>
        </template>
