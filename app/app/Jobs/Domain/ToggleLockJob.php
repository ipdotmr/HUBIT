...
class ToggleLockJob implements ShouldQueue
{
    ...
    public function handle(ProvisioningLogService $logger): void
    {
        ...
        $logger->start('domain.lock.toggle', $this->domain, [
        ...
        try {
            ...
            $result = $registrar->toggleLock($this->domain->domain, $this->enabled);

            if ($result['success']) {
                $this->domain->update(['lock_enabled' => $this->enabled]);

                $logger->success('domain.lock.toggle', $this->domain, [
                    'enabled' => $this->enabled,
                    'result' => $result,
                ]);
            } else {
                throw new \Exception($result['error'] ?? 'Failed to toggle lock');
            }
        } catch (\Exception $e) {
            $logger->failed('domain.lock.toggle', $this->domain, [
                'error' => $e->getMessage(),
                'attempt' => $this->attempts(),
            ]);

            if ($this->attempts() >= $this->tries) {
                Log::error('Lock toggle failed permanently', [
    ...
    {
        return 'lock_'.$this->domain->id.'_'.($this->enabled ? '1' : '0');
    }
